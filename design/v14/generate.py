...
def shell(s,mode,page):
 ...
 s.link('cpm-host-switch.svg');s.rect(1263,33,177,46,'#22222c',23);s.icon(1281,45,'host','#c3c2ce');s.text(1316,62,'PS4 · 客厅',15,WHITE,500);s.text(1413,61,'⌄',17,MUTED);s.endlink()
 s.link('cpm-settings-general.svg');s.rect(1475,34,46,46,'#22222c',23);s.icon(1487,46,'settings','#d0cfda');s.endlink()
 return 66,1462
...
library('controller');tasks('controller');library('controller',True)
...
files=[('游戏库','cpm-library-controller.svg'),('安装任务','cpm-tasks-controller.svg'),('切换主机','cpm-host-switch.svg'),('全屏设置','cpm-settings-general.svg'),('游戏库设置','cpm-settings-library.svg'),('添加主机','cpm-add-host.svg')]
(OUT/'index.html').write_text('<!doctype html><meta charset="utf-8"><title>CPM SVG 设计稿 v14</title><style>body{margin:0;background:#0d0d13;color:#aaa;font:13px system-ui}header{padding:12px 24px;display:flex;gap:24px}a{color:#ccc;text-decoration:none}iframe{width:100%;height:calc(100vh - 44px);border:0}</style><header>'+''.join(f'<a target="screen" href="{f}">{t}</a>' for t,f in files)+'</header><iframe name="screen" src="cpm-library-controller.svg"></iframe>')
